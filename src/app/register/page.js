...
export default function RegisterPage() {
  ...
  const validatePassword = (pass) => {
    const errs = [];
    if (pass.length < 6) errs.push("At least 6 characters");
    if (!/[A-Z]/.test(pass)) errs.push("At least 1 uppercase letter");
    if (!/[a-z]/.test(pass)) errs.push("At least 1 lowercase letter");
    return errs;
  };
      ...
      <div className="w-full max-w-md">
        <div className="bg-white rounded-2xl shadow-xl border border-slate-100 p-8 md:p-10">
          <div className="text-center mb-8">
            <div className="w-14 h-14 bg-gradient-to-br from-teal-500 to-blue-600 rounded-2xl flex items-center justify-center text-2xl mx-auto mb-3 shadow-lg">
              👨‍⚕️
            </div>
            <h1 className="text-2xl font-bold text-slate-900" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
              Register
            </h1>
            <p className="text-slate-500 text-sm mt-1">Create your DocAppoint account</p>
          </div>
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            ...
            <div>
              ...
              <input type="url" name="photoURL" placeholder="https://example.com/photo.jpg"
                value={form.photoURL} onChange={handleChange} className="input-field" />
            </div>
            <div>
              ...
              <div className="mt-2 space-y-1">
                {[
                  ...
                  { label: "1 lowercase letter", ok: /[a-z]/.test(form.password) },
                ].map((rule) => (
                  <p key={rule.label} className={`text-xs flex items-center gap-1.5 ${form.password ? (rule.ok ? "text-green-600" : "text-red-400") : "text-slate-400"}`}>
                    <span>{form.password ? (rule.ok ? "✅" : "❌") : "○"}</span>
                    {rule.label}
                  </p>
                ))}
              </div>
